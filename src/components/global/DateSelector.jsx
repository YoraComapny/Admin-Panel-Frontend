  ...
  return (
    <div className="  mb-8 flex gap-3 justify-center items-center ">
      {generateDates(selectedDate).map((date, index) => (
        <div
          key={index}
          className={` flex flex-col font-semibold text-sm p-1 cursor-pointer rounded-full items-center  justify-center w-[6.5rem] h-[4rem]  transition-colors ${
            date.toDateString() === selectedDate.toDateString()
              ? "bg-blue-600 text-white font-bold"
              : "bg-[#eaeaea] text-[#9e9494]"
          }`}
      ...
      <div className="relative">
        <FaCalendarAlt
          className="w-[6.5rem] h-[4rem] text-md p-4 rounded-full bg-[#9e9494] text-white cursor-pointe transition-colors"
